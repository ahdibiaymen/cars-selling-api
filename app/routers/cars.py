...
from app.models import CarModelFull, CarModelBase
...
async def list_all(request: Request, page: int = 1, page_limit: int = Settings.PAGE_LIMIT, min_price: int = 0,
                   max_price: int = 10000, brand: Optional[str] = None):
    ...
    cars = (request.app.mongodb[Settings.COLLECTION_NAME]
            .find(query)
            .skip(skip_docs)
            .limit(page_limit))
...
async def get_one_car(request: Request, car_id: str):
    ...
    car = await request.app.mongodb[Settings.COLLECTION_NAME].find_one({'_id': car_id})
    if not car:
        return HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Car not found')

    return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(CarModelFull(**car)))
...
async def add_new_car(request: Request, new_car: CarModelBase):
    ...
    doc = await request.app.mongodb[Settings.COLLECTION_NAME].insert_one(new_car)
    if not doc:
        return HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail='Request not accepted')

    new_car = await request.app.mongodb[Settings.COLLECTION_NAME].find_one({'_id': doc.inserted_id})
    return JSONResponse(status_code=status.HTTP_201_CREATED, content=jsonable_encoder(CarModelFull(**new_car)))
...
async def update_car_information(request: Request, car_data: CarModelFull):
    ...
    update_results = await request.app.mongodb[Settings.COLLECTION_NAME].update_one({'_id': car_id}, {'$set': car_data})

    if update_results.matched_count == 0:
        return HTTPException(status_code=status.HTTP_200_OK, detail='No matched records')
...
async def delete_one_car(request: Request, car_id: str):
    ...
    if not car_id:
        return HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Please provide a valid ObjectID')

    delete_status = await request.app.mongodb[Settings.COLLECTION_NAME].delete_one({'_id': car_id})
